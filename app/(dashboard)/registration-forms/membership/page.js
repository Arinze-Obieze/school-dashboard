 ...
 const membershipData = {
  ...
  const isStepValid = () => {
    ...
    if (step === 5) {
      ...
      return (
        formData.degreeCertificates && formData.trainingCertificate && formData.workExperienceProof && formData.cpdCertificates && formData.passportPhoto && formData.declarationChecked && formData.declarationDate
      );
    }
    return true;
  };
    ...
    try {
      ...
      const paymentRes = await fetch('/api/payments/create', {
        ...
        body: JSON.stringify({
          ...
          txRef: `MEMBERSHIP-${user.uid}-${Date.now()}`,
          description: 'Membership Examination Registration Fee',
  ...
  return (
    <>
      <PaymentAmountModal
        ...
        title="Membership Examination Fee"
        description="Enter the amount for Membership Examination registration"
      />
      {showSuccess ? (
        <MembershipSuccess />
      ) : (
                  ...
                  {step === totalSteps - 1 && (
                    ...
                    >
                      {loading ? (
                        ...
                        paymentSuccess ? 'Submit Membership Application' : 'Proceed to Payment'
                      )}
                    </button>
                  )}
                </form>
